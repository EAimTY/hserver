...
impl Service<Request<Incoming>> for EchoPeer {
    ...
    fn call(&mut self, request: Request<Incoming>) -> Self::Future {
        ready(Ok(peer_response(&request)))
    }
}
...
impl Service<Request<Incoming>> for Failing {
    ...
    fn call(&mut self, _request: Request<Incoming>) -> Self::Future {
        ready(Err(std::io::Error::other("service failed")))
    }
}
...
async fn with_timeout<F: Future>(future: F) -> Result<F::Output, Box<dyn Error + Send + Sync>> {
    timeout(TEST_TIMEOUT, future).await.map_err(Into::into)
}
...
async fn http1_serves_keep_alive_requests_with_peer_info() -> TestResult {
    let (mut client, server_stream) = duplex(64 * 1024);
...
async fn http2_serves_request_with_peer_info() -> TestResult {
    ...
    let (request_sender, h2_connection) = with_timeout(h2::client::handshake(client)).await??;
    tokio::spawn(async move {
...
async fn graceful_shutdown_completes_in_flight_request() -> TestResult {
    ...
    tokio::spawn(async move {
        ...
        let result = match future::select(&mut shutdown_wait, &mut connection).await {
